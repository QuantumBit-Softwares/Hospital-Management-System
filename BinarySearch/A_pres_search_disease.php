...
//access then display to table
    ...
    global $con;
    //sort
    $query = "select * from prestb ORDER BY disease";
    $result = mysqli_query($con,$query);
    while ($row = mysqli_fetch_array($result)){
        $doctor =  strtoupper($row['doctor']);
        $pid =  strtoupper($row['pid']);
        $ID =  strtoupper($row['ID']);
        $fname =  strtoupper($row['fname']);
        $lname =  strtoupper($row['lname']);
        $appdate =  strtoupper($row['appdate']);
        $apptime =  strtoupper($row['apptime']);
        $disease =  strtoupper($row['disease']);
        $allergy =  strtoupper($row['allergy']);
        $pres =  strtoupper($row['prescription']);
...
function myFunction() {
  ...
  //key is 'filter'
     ...
     //assigning first value to txtValue and txtValue1 AND td and td1
       ...
       td = tr[i].getElementsByTagName("td")[7]; //search input by user (choose from one, two, three, four, four)

       td1 = tr[i].getElementsByTagName("td")[7];  //search input by user (choose 1, 2, 3, 4, 4)
